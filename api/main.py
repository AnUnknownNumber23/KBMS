...
@app.get("/api/admin/vectors", tags=["系统管理"])
async def list_vectors(limit: int = 100):
    """浏览向量库中的文档块"""
    return await vector_store.list_all(limit=limit)


@app.delete("/api/admin/source", tags=["系统管理"])
async def delete_source(file_path: str):
    """按文件路径删除向量库和图谱中的该文件数据"""
    import hashlib
    doc_id = hashlib.sha256(file_path.encode()).hexdigest()[:16]
    vs_deleted = await vector_store.delete_by_doc_id(doc_id)
    kg_deleted = await knowledge_graph.delete_by_source(file_path)
    return {"file_path": file_path, "vectors_deleted": vs_deleted, "graph_entities_deleted": kg_deleted}
...
_bot_handler: Any = None


def _get_bot_handler():
    """懒加载 Bot 处理器，复用已初始化的基础设施"""
    global _bot_handler
    if _bot_handler is None:
        from integrations.feishu_bot import FeishuBotHandler, FeishuClient
        client = FeishuClient(
...
@app.post("/api/bot/feishu", tags=["飞书集成"])
async def feishu_webhook(body: dict):
    """
    飞书 Bot 事件回调 — 在飞书开放平台配置此 URL 作为事件订阅地址
