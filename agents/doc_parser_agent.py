...
class DocParserAgent:
    ...
    async def _parse_image(self, file_path: str) -> list[str]:
        """图片解析: OCR + LLM 视觉理解（视觉不可用时仅用 OCR）"""
        texts: list[str] = []
        ...
        try:
            ...
            if not texts:
                texts.append(f"[图片 OCR 结果] {ocr_text}" if ocr_text.strip() else f"[图片无法识别] {file_path}")
        return texts
    ...
    async def _describe_image_with_llm(self, image: Any) -> str:
        """调用 LLM 多模态能力描述图片内容。
        如果 LLM 不支持视觉（如 DeepSeek），抛出异常，由上层回退到 OCR。"""
    ...
    @staticmethod
    def _parse_xls(file_path: str) -> list[str]:
        """老格式 .xls → 用 xlrd 解析"""
        try:
            import xlrd
            ...
            return [f"[Excel(.xls) 解析失败] {file_path}"]
